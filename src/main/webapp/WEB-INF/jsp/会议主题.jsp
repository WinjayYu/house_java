                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-heading">
                        <div class="btn-group pull-left">
                            <a href="javascript:void(0)"  id="newForum" class="btn btn-outline btn-primary btn-lg"
                               role="button">新建主题</a>
                        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="subjectModalLabel">操作</h4>
                </div>
                <div class="modal-body ">
                    <form id="subjectForm" method="post" action="forum/subject/save" class="form-horizontal" role="form">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="subjectSave" class="btn btn-primary">保存</button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
        ...
        fn: {
            init: function () {
                ...
                $("#c_search").click(function () {
                    subjectList.v.dTable.ajax.reload();
                })
                ...
                $("#newForum").click(function(){
                    self.showModal("subjectModal","添加会议主题");
                })
            ...
            dataTableInit: function () {
                subjectList.v.dTable = $health.dataTable($('#dataTables'), {
                    "processing": true,
                    "serverSide": true,
                    "searching":false,
                    "ordering": false,
                    "ajax": {
                        "url": "forum/subject/list",
                        "type": "POST"
                    },
                    "columns": [
                        {"data": "id"},
                    ...
                    "footerCallback": function( tfoot, data, start, end, display ) {
                        subjectList.v.list=data
                    },
                    "rowCallback": function (row, data) {
                        $('td', row).last().find(".disable").click(function(){
                            subjectList.fn.disable([data.id]);
                        })
                        $('td', row).last().find(".edit").click(function(){
                            subjectList.fn.edit(data.id);
                        })
                        $('td', row).last().find(".tagPop").click(function(){
                            subjectList.fn.tagPop(data.id);
                        })
                    },
            ...
            dropperInit: function () {
                $("#subjectModal" + " .dropped").dropper({
                    postKey: "file",
                    action: "gen/save/image",
                    postData: {thumbSizes: '480x320'},
                    label: "把文件拖拽到此处",
                    maxSize: 204857
                }).on("fileComplete.dropper", subjectList.fn.onFileComplete)
                        .on("fileError.dropper", subjectList.fn.onFileError);
            },
            onFileComplete: function (e, file, response) {
                if (response.status == '200') {
                    subjectList.fn.viewImage(response.data);
                } else {
            ...
            disable:function(ids){
                if (ids.length > 0) {
                    $health.optNotify(function () {
                        $health.ajax("forum/subject/delete", {ids:JSON.stringify(ids)}, function (result) {
                            subjectList.fn.responseComplete(result);
                        })
                    },"你确定要删除吗？","确定");
                }
            },
            ...
            edit: function (id) {
                subjectList.fn.showModal("subjectModal","修改会议主题");
                var items = subjectList.v.list;
                $.each(items, function (index, item) {
                    if (item.id == id) {
                        for (var key in item) {
                            var element = $("#subjectForm :input[name=" + key + "]")
                            if (element.length > 0) {
                                element.val(item[key]);
                            }
                            if (key == "bgImg") {
                                if (item.bgImg) {
                                    subjectList.fn.viewImage(item.bgImg);
                                } else {
                                    subjectList.fn.clearImageView();
                                }
                            }
                        }
                    }
            ...
            save: function () {
                ...
                $("#subjectForm").ajaxSubmit({
                    ...
                    success: function (result) {
                        subjectList.fn.responseComplete(result,true,$("#subjectModal"));
                    },
            ...
            responseComplete: function (result,action,modal) {
                if (result.status == "200") {
                    ...
                    if(action){
                        subjectList.v.dTable.ajax.reload(null, false);
                    }else{
                        subjectList.v.dTable.ajax.reload();
                    }
