        ...
        fn: {
            init: function () {
                ...
                $("#newForum").click(function(){
                    self.showModal("forumModal","添加管理员帐号");
                })
                    ...
                    "columnDefs": [
                        {
                            ...
                            "</button>" +
                            "&nbsp;&nbsp;" +
                            "<button   title='添加联会主席'  class='btn btn-danger btn-circle userPop'>" +
            ...
            edit: function (id) {
                forumList.fn.showModal("forumModal","修改管理员帐号");
                var items = forumList.v.list;
                $.each(items, function (index, item) {
                    if (item.id == id) {
                        for (var key in item) {
                            var element = $("#forumForm :input[name=" + key + "]")
                            if (element.length > 0) {
                                element.val(item[key]);
                            }
                            if (key == "bgImgUrl") {
                                if (item.bgImgUrl) {
                                    forumList.fn.viewImage(item.bgImgUrl);
                                } else {
                                    forumList.fn.clearImageView();
                                }
                            }
                        }
                    }
            ...
            userPop:function(id){
                $('#popModal').modal('show').find('.modal-content').load("user/pop?forumId="+id);
            },
            save: function () {
