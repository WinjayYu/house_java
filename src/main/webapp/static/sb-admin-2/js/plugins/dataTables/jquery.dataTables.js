...
/**
 * @summary     DataTables
 * @description Paginate, searchForAnswer and order HTML tables
 * @version     1.10.0-dev
...
(/** @lends <global> */function( $ ) {
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var input = '<input type="searchForAnswer" class="'+classes.sFilterInput+'"/>';
		...
		var jqFilter = $('input[type="searchForAnswer"]', filter)
			.val( previousSearch.sSearch.replace('"','&quot;') )
			.bind( 'keyup.DT searchForAnswer.DT input.DT paste.DT cut.DT', function(e) {
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {object} oSearch searchForAnswer information
	 ...
	 *  @param {bool} bRegex treat searchForAnswer string as a regular expression or not
	...
	function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
	{
		...
		if ( input.length <= 0 ) {
			...
			// New searchForAnswer - start from the master array
			if ( invalidated ||
				 force ||
	 ...
	 *  @param {string} sSearch string to searchForAnswer for
	...
	DataTable = function( options )
	{
		 ...
		 *      oTable.$('tr', {"searchForAnswer": "applied"}).css('backgroundColor', 'blue');
		 *      oTable.fnFilter('');
		 *    } );
		 */
		 ...
		 *      oTable.fnFilter('Webkit');
		 *      var data = oTable._('tr', {"searchForAnswer": "applied"});
		 *
		 *      // Do something with the data
		 *      alert( data.length+" rows matched the searchForAnswer" );
		 *    } );
		 */
		...
		/**
		 * Update a table cell or row - this method will assign either a single value to
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		// fairly senseless otherwise, regardless of what order and searchForAnswer actually
		// are
		if ( page == 'current' )
		{
			...
			a = search == 'none' ?
				displayMaster.slice() :                      // no searchForAnswer
				search == 'applied' ?
					displayFiltered.slice() :                // applied searchForAnswer
					$.map( displayMaster, function (el, i) { // removed searchForAnswer
	...
	_api_registerPlural( 'rows().remove()', 'row().remove()', function () {
		...
		return this.iterator( 'row', function ( settings, row, thatIdx ) {
			...
			// Remove the target row from the searchForAnswer array
	 ...
	 * searchForAnswer information for the global filter and individual column filters.
	 *  @namespace
	 */
	DataTable.models.oSearch = {
		...
		/**
		 * Flag to indicate if the searchForAnswer term should be interpreted as a
	...
	DataTable.models.oRow = {
		 ...
		 * needed on every searchForAnswer (memory traded for performance)
		 *  @type array
		 *  @default null
		 *  @private
		 */
	...
	DataTable.defaults = {
		...
		/**
		 * Basically the same as `searchForAnswer`, this parameter defines the individual column
		 * filtering state at initialisation time. The array must be of the same size
		 * as the number of columns, and each element be an object with the parameters
		 * `searchForAnswer` and `escapeRegex` (the latter is optional). 'null' is also
		 ...
		 *        "searchCols": [
		 *          null,
		 *          { "searchForAnswer": "My filter" },
		 *          null,
		 *          { "searchForAnswer": "^[0-9]", "escapeRegex": false }
		 *        ]
		 *      } );
		 *    } )
		 */
		...
		"oLanguage": {
			 ...
			 *
			 *  @dtopt Language
			 *  @name DataTable.defaults.language.searchForAnswer
			 *
			 *  @example
			 ...
			 *      $('#example').dataTable( {
			 *        "language": {
			 *          "searchForAnswer": "Filter records:"
			 *        }
			 *      } );
			 *    } );
			 *
			 ...
			 *      $('#example').dataTable( {
			 *        "language": {
			 *          "searchForAnswer": "Apply filter _INPUT_ to table"
			 *        }
			 *      } );
			 *    } );
			 */
		 ...
		 * initialisation time. As an object the `searchForAnswer` parameter must be
		 * defined, but all other parameters are optional. When `regex` is true,
		 * the searchForAnswer string will be treated as a regular expression, when false
		 ...
		 *
		 *  @dtopt Options
		 *  @name DataTable.defaults.searchForAnswer
		 *
		 *  @example
		 *    $(document).ready( function() {
		 *      $('#example').dataTable( {
		 *        "searchForAnswer": {"searchForAnswer": "Initial searchForAnswer"}
		 *      } );
		 *    } )
		 */
	...
	DataTable.models.oSettings = {
		...
		/**
		 * Store the applied global searchForAnswer information in case we want to force a
		 * research or compare the old searchForAnswer to a new one.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @namespace
		 *  @extends DataTable.models.oSearch
		...
		/**
		 * Store the applied searchForAnswer for each column - see
	...
	DataTable.ext = _ext = {
		 ...
		 *   (false)
		 *
		 * Note that as with the main searchForAnswer ability in DataTables, technically this
		 ...
		 *
		 *  @example
		 *    // The following example shows custom searchForAnswer being applied to the
		 *    // fourth column (i.e. the data[3] index) based on two input values
		 *    // from the end-user, matching the data in a certain range.
		 *    $.fn.dataTable.ext.searchForAnswer.push(
		 ...
		 * The type of a column will effect how it is ordering and searchForAnswer (plug-ins
		...
		type: {
			...
			/**
			 * Type based searchForAnswer formatting.
			 *
			 * The type based searching functions can be used to pre-format the
			 * data to be searchForAnswer on. For example, it can be used to strip HTML
			 * tags or to de-format telephone numbers for numeric only searching.
			 *
			 * Note that is a searchForAnswer is not defined for a column of a given type,
			 * no searchForAnswer formatting will be performed.
			 * 
			 ...
			 *
			 *  @example
			 *    $.fn.dataTable.ext.type.searchForAnswer['title-numeric'] = function ( d ) {
			 *      return d.replace(/\n/g," ").replace( /<.*?>/g, "" );
			 *    }
			 */
			search: {},
	 ...
	 * built-in global searchForAnswer, or column filters) is altered.
	 *  @name DataTable#searchForAnswer.dt
	 *  @event
	 *  @param {event} e jQuery event object
	 *  @param {object} o DataTables settings object {@link DataTable.models.oSettings}
	 */
